<div>
    ...
    @if (Auth::user()->hasRole('admin'))
        <div class="flex justify-between mb-4  sm:-mt-4">
            <div class="font-bold text-md tracking-tight text-md text-black  mt-2 uppercase">Admin / Manage Event</div>
        </div>
    @else
        <div class="flex justify-between mb-4  sm:-mt-4">
            <div class="font-bold text-md tracking-tight text-md text-black  mt-2 uppercase">Event Manager / Manage Event</div>
        </div>
    @endif
    ...
    <hr class="border-gray-200 my-4">
        <div x-data="{ open: false }">
            ...
            <button @click="open = true" class="bg-blue-500 text-white text-sm px-3 py-2 rounded hover:bg-blue-700 mb-4">
            ...
            <div x-cloak x-show="open" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
                <div @click.away="open = true" class="bg-white p-6 rounded-lg shadow-lg max-w-md mx-auto">
                    ...
                    <div class="mb-4">
                        ...
                        @else
                            <form action="{{ route('event_manager.event.store') }}" method="POST" class="">
                        @endif
                            @csrf
                            <div class="mb-4  ">
                            ...
                            <div class="mb-2">
                                    ...
                                    <select id="type_of_scoring" name="type_of_scoring" class="shadow appearance-none border rounded w-full py-2 px-3 text-black leading-tight focus:outline-none focus:shadow-outline @error('type_of_scoring') is-invalid @enderror" required>
                                            ...
                                            <option value="ranking(H-L)">By Ranking (Highest is the winner)</option>
                                            <option value="ranking(L-H)">By Ranking (Lowest is the winner)</option>
                                    </select>
                                    ...
                                    <div x-cloak x-show="open" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
                                        <div @click.away="open = true" class="bg-white p-6 rounded-lg shadow-lg max-w-md mx-auto">
                                            ...
                                            <div class="mb-4">
                                                ...
                                                @else      
                                                    <form id="updateEventForm" action="{{ route('event_manager.event.update', $event->id )}}" method="POST" class="">
                                                @endif
                                                        @csrf
                                                        @method('PUT')
                                                        <div class="mb-4">
