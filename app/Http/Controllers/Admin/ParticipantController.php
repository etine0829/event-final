...
public function store(Request $request)
    ...
    if (Auth::user()->hasRole('admin')) {
        ...
        try {
            Participant::create($validatedData);
            return redirect()->route('event_manager.participant.index')
                ->with('success', 'Participant created successfully.');
        } catch (Exception $e) {
            Log::error('Participant creation failed: ' . $e->getMessage());
            return redirect()->back()->with('error', 'Failed to create participant.');
...
public function update(Request $request, Participant $participant)
{
    if(Auth::user()->hasRole('admin')){
        ...
        return redirect()->route('event_manager.participant.index')->with('success', 'Participant updated successfully.');
    }
}
...
{
    // Check if the user has either 'admin' or 'event_manager' role
    if (Auth::user()->hasRole('admin') || Auth::user()->hasRole('event_manager')) {
        
        // Check for associated records (e.g., scorecards)
        if ($participant->scorecards()->exists()) {
            // If associated records exist, return an error message
            $route = Auth::user()->hasRole('admin') ? 'admin.participant.index' : 'event_manager.participant.index';
            
            return redirect()->route($route)
                ->with('error', 'Participant cannot be deleted because they have associated records.');
        }

        // Proceed with deletion if no associated records exist
        $participant->delete();
